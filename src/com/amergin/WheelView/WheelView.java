...
public class WheelView extends View {
	...
	/** Current value & label text colorF0FF6347 */
	private static final int VALUE_TEXT_COLOR = 0xFFB273F3;

	/** Items text color */
	private static final int ITEMS_TEXT_COLOR = 0xFF000000;

	/** Top and bottom shadows colors */
	private static final int[] SHADOWS_COLORS = new int[] { 0x15a12dfa,
			0x03AAAAAA, 0x00a12dfa };

	/** Additional items height (is added to standard text item height) */
	private static final int ADDITIONAL_ITEM_HEIGHT = 5;  //原来15

	/** Text size */
	private static final int TEXT_SIZE = 32;  //原来24

	/** Top and bottom items offset (to hide that) */
	private static final int ITEM_OFFSET = (TEXT_SIZE / 5)+2;
	...
	private int calculateLayoutWidth(int widthSize, int mode) {
		...
		if (maxLength > 0) {
			float textWidth = (float) java.lang.Math.ceil(Layout.getDesiredWidth("0",
					itemsPaint));
		...
		if (label != null && label.length() > 0) {
			labelWidth = (int) java.lang.Math.ceil(Layout.getDesiredWidth(label,
					valuePaint));
		}
